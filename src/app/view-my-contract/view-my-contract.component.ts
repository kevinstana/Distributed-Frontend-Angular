...
export class ViewMyContractComponent {
  ...
  constructor(
        ...
        this.router.navigateByUrl('/login');
          ...
          if (!this.isLawyer && !this.isClient) {
            this.router.navigateByUrl('/home');
              ...
              this.userService.getMyContract(this.myId!).subscribe({
              next: (data) => {
                this.myContract = data;
                this.member1 = this.myContract.members[0];
                this.member2 = this.myContract.members[1];
                this.member3 = this.myContract.members[2];
                this.member4 = this.myContract.members[3];
            },
            error: (err) => {
    ...
    OnSubmit(): void {
      this.contractService.answerContract(this.myId!).subscribe({
        next: (data) => {
